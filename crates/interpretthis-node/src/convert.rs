...
fn value_to_js_inner<'env>(env: &'env Env, value: &Value) -> Result<Unknown<'env>> {
    match value {
        ...
        // A JS `Set` preserves insertion order, so project the elements in the
        // set's stored CPython iteration order to match what the interpreter shows.
        Value::Set(_) => {
        ...
        Value::Decimal(d, kind) => {
            // Cross as the exact `str()` form, which restores negative zero and
            // spells out Infinity / NaN.
